...
import { GiftIncomingModal } from "@/components/gifts/gift-incoming-modal";
    ...
    >
      {brand?.heroVideoUrl ? (
        <div className="relative h-40 w-full overflow-hidden">
          <video src={brand.heroVideoUrl} className="h-full w-full object-cover" autoPlay muted loop playsInline />
          ...
          <div className="flex-1">
            ...
            <p className="text-sm opacity-80">
              ...
              <span className="inline-flex items-center gap-1">
                {menu.table.tableIconEmoji || menu.activeSession?.tableIconEmoji || "🪑"}
                {locale === "ar" ? `طاولة ${menu.table.number}` : `Table ${menu.table.number}`}
                {(menu.table.label || menu.activeSession?.tableLabel) &&
                  ` — ${menu.table.label || menu.activeSession?.tableLabel}`}
              </span>
              {menu.table.zone && ` · ${menu.table.zone}`}
            </p>
            {(menu.table.minimumSpendAmount != null && menu.table.minimumSpendAmount > 0) ||
            menu.activeSession ? (
              <div className="mt-2 space-y-1">
                {menu.activeSession && (
                  <p className="text-sm font-medium">
                    {locale === "ar"
                      ? brand?.welcomeText || `مرحباً ${menu.activeSession.customerName}`
                      : brand?.welcomeTextEn || `Welcome ${menu.activeSession.customerName}`}
                  </p>
                )}
                {(() => {
                  const min =
                    menu.activeSession?.minimumSpendAmount ??
                    menu.table.minimumSpendAmount;
                  if (min == null || min <= 0) return null;
                  return (
                    <p className="inline-flex items-center gap-1 rounded-full bg-white/20 px-2 py-0.5 text-xs">
                      {locale === "ar"
                        ? `حد أدنى: ${min} ريال`
                        : `Min. order: ${min} SAR`}
                    </p>
                ...
                {menu.reservation && (
                  <p className="text-xs opacity-90">
                    {locale === "ar" ? "حجز" : "Reservation"}: {menu.reservation.time}
                    {menu.reservation.occasion ? ` · ${menu.reservation.occasion}` : ""}
                  </p>
                )}
              </div>
            ) : null}
      ...
      <div className="fixed bottom-20 left-0 right-0 z-30 mx-auto flex max-w-lg justify-center gap-2 px-4">
        {menu.giftsEnabled && (
          <Link
            href={`/gift/${tableId}?lang=${locale}`}
            className="flex items-center gap-1 rounded-full bg-amber-500 px-4 py-2 text-xs font-medium text-white shadow"
          >
            <Gift className="h-3.5 w-3.5" />
            {locale === "ar" ? "إهداء" : "Gift"}
          </Link>
        )}
        {[
      ...
      {menu.giftsEnabled && (
        <GiftIncomingModal tableId={tableId} locale={locale} />
      )}
    </div>
  );
}
